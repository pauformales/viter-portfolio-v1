...
const RecentWorkListTable = ({ setItemEdit, setIsModal }) => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/rest/v1/controllers/developer/recent-work/search.php`, // url search
        `/rest/v1/controllers/developer/recent-work/page.php?start=${pageParam}`, // list page
  ...
  return (
    <>
                ...
                {page.data.map((item, key) => {
                  return (
                    <tr key={key} className="relative group cursor-pointer">
                      ...
                      <td>{item.mainrecentwork_title}</td>
                      <td>{item.mainrecentwork_description}</td>

                      <td colSpan="100%">
                        {item.mainrecentwork_is_active == 1 ? (
                          <>
      ...
      {store.archive && (
        <ModalArchive
          endpoint={`/rest/v1/controllers/developer/recent-work/active.php?mainrecentworkid=${id}`}
          msg={`Are you sure want to archive this record?`}
          successMsg={`Successfully Archived`}
          queryKey={`recent-work-list`}
        />
      )}

      {store.delete && (
        <ModalDelete
          endpoint={`/rest/v1/controllers/developer/recent-work/recent-work.php?mainrecentworkid=${id}`}
          msg={`Are you sure want to delete this record?`}
          successMsg={`Successfully Delete.`}
          item={dataItem.mainrecentwork_title}
          queryKey={`recent-work-list`}
        />
      )}
...
export default RecentWorkListTable;
