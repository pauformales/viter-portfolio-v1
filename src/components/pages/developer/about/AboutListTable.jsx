...
const AboutListTable = ({ setItemEdit, setIsModal }) => {
  ...
  const {
    ...
    queryKey: ["about-list", search.current.value, store.isSearch, isActive],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/rest/v1/controllers/developer/about/search.php`, // url search
  ...
  return (
    <>
                ...
                {page.data.map((item, key) => {
                  return (
                    <tr key={key} className="relative group cursor-pointer">
                      ...
                      <td>{item.mainabout_title}</td>
                      <td>{item.mainabout_description}</td>

                      <td colSpan="100%">
                        {item.mainabout_is_active == 1 ? (
                          <>
      ...
      {store.archive && (
        <ModalArchive
          endpoint={`/rest/v1/controllers/developer/about/active.php?mainaboutid=${id}`}
      ...
      {store.delete && (
        <ModalDelete
          endpoint={`/rest/v1/controllers/developer/about/about.php?mainaboutid=${id}`}
          msg={`Are you sure want to delete this record?`}
          successMsg={`Successfully Delete.`}
          item={dataItem.mainabout_title}
          queryKey={`about-list`}
        />
      )}
