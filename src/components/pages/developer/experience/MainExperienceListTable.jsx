...
const MainExperienceListTable = ({ setItemEdit, setIsModal }) => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/rest/v1/controllers/developer/experience/search.php`,
        `/rest/v1/controllers/developer/experience/page.php?start=${pageParam}`,
  ...
  return (
    <>
              ...
              <React.Fragment key={key}>
                {page.data?.map((item, key) => (
                  <tr key={key} className="relative group cursor-pointer">
                    ...
                    <td>{item.mainexperience_title}</td>
                    <td>{item.mainexperience_category}</td>
                    <td>{item.mainexperience_description}</td>
                    <td colSpan="100%">
                      {item.mainexperience_is_active == 1 ? (
      ...
      {store.archive && (
        <ModalArchive
          endpoint={`/rest/v1/controllers/developer/experience/active.php?mainexperienceid=${id}`}
          msg={`Are you sure want to archive this record?`}
          successMsg={`Successfully Archived`}
          queryKey={`mainexperience`}
        />
      )}

      {store.delete && (
        <ModalDelete
          endpoint={`/rest/v1/controllers/developer/experience/experience.php?mainexperienceid=${id}`}
          msg={`Are you sure want to delete this record?`}
          successMsg={`Successfully Delete.`}
          item={dataItem.mainexperience_title}
          queryKey={`mainexperience`}
        />
      )}
...
export default MainExperienceListTable;
