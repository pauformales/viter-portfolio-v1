...
const TestimonialsListTable = ({ setItemEdit, setIsModal }) => {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `/rest/v1/controllers/developer/testimonials/search.php`,
        `/rest/v1/controllers/developer/testimonials/page.php?start=${pageParam}`,
  ...
  const handleArchive = (item) => {
    setDataItem(item);
    setId(item.maintestimonials_aid);
  ...
  return (
    <>
                ...
                {(page?.data || []).map((item, key) => {
                  return (
                    <tr key={key} className="relative group cursor-pointer">
                      ...
                      <td>{`${item.maintestimonials_first_name} ${item.maintestimonials_last_name}`}</td>
                      <td>{item.maintestimonials_email}</td>
                      <td>{item.maintestimonials_description}</td>
                      <td colSpan="100%">
                        {item.maintestimonials_is_active == 1 ? (
      ...
      {store.archive && (
        <ModalArchive
          endpoint={`/rest/v1/controllers/developer/testimonials/active.php?maintestimonialsid=${id}`}
      ...
      {store.delete && (
        <ModalDelete
          endpoint={`/rest/v1/controllers/developer/testimonials/testimonials.php?maintestimonialsid=${id}`}
          msg={`Are you sure want to delete this record?`}
          successMsg={`Successfully Delete.`}
          item={dataItem?.maintestimonials_first_name}
          queryKey={`testimonials-list`}
        />
      )}
...
export default TestimonialsListTable;
